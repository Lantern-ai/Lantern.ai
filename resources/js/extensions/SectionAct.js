...
export const SectionAct = Node.create({
    name: 'sectionAct',
    ...
    parseHTML() {
        return [{ tag: 'div[data-type="section-act"]' }];
    },
    ...
    renderHTML({ HTMLAttributes }) {
        return [
            ...
            mergeAttributes(HTMLAttributes, {
                'data-type': 'section-act',
                class: 'screenplay-section-act',
            }),
            0,
        ];
    },
    ...
    addKeyboardShortcuts() {
        return {
            Enter: ({ editor }) => {
                ...
                if ($from.parent.type.name === 'sectionAct') {
                    return editor
                        .chain()
                        .insertContentAt($from.after(), {
                            type: 'action',
                            attrs: { id: genId() }, // ensure the new Action has a stable id
                            content: [],
                        })
                        .focus($from.after() + 1)
                        .run();
